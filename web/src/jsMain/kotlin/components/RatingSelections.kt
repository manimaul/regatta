...
fun RatingSelections(
    ...
    RatingSelections(
        ...
        boatType = boat.ratingType,
        phrfRating = boat.phrfRating ?: ratingDefault,
        orcCertificate = if (selectMode) boat.orcCerts.findPreferred() else null,
        certs = boat.orcCerts,
        onOrc = onOrc,
...
fun RatingSelections(
    ...
    boatType: RatingType,
    phrfRating: Int?,
    hideRatingSelector: Boolean = false,
    hideCruising: Boolean = false,
    orcCertificate: OrcCertificate? = null,
    certs: List<OrcCertificate> = emptyList(),
    onOrc: ((Action, OrcCertificate) -> Unit)? = null,
    typeChange: (RatingType, Int) -> Unit,
) {
    if (!hideRatingSelector) {
        P {
            Label(attrs = {
                classes(AppStyle.marginEnd)
            }) { B { Text("Rating Type") } }
            RgDropdown(RatingType.entries.filter {
                if (it.isORC) {
                    certs.isNotEmpty() || !selectMode
                } else {
                    !hideCruising || it == RatingType.PHRF
                }
            }, boatType, { it.label }) {
                typeChange(it, phrfRating ?: ratingDefault)
            }
        }
    }
    when (boatType) {
        RatingType.ORC -> {
            P { OrcInfo(selectMode = selectMode, selected = orcCertificate, certs = certs) { c, a -> onOrc?.invoke(c, a) } }
        }
...
fun OrcInfo(
    ...
    if (selectMode) {
        ...
        B {
            Text("ORC Certificate")
        }
        if (certs.isEmpty()) {
            P {
                Text("Something went wrong - no certificates available")
            }
        } else {
            RgDropdown(
                items = certs,
                selectedItem = selected ?: run { certs.first() },
                name = {
                    it.label()
                },
                handler = {
        ...
        OrcViewAndAdd(certs = certs, onOrc = onOrc)
    }
}
